...
static void atmel_ts_work_func(struct work_struct *work)
{
	...
	uint8_t loop_i, loop_j, report_type, msg_byte_num = 8, finger_report;
	memset(data, 0x0, sizeof(data));
			...
			if (data[1] & 0x20) {
				...
				if (((ts->finger_pressed >> report_type) & 1) == 1) {
					...
					if (!ts->first_pressed) {
						if (!ts->finger_count)
							ts->first_pressed = 1;
						printk(KERN_WARNING "E%d@%d,%d\n", report_type + 1,
								ts->finger_data[report_type].x, ts->finger_data[report_type].y);
					}
					if (ts->id->version >= 0x20 && ts->pre_data[0] == 0) {
						if (ts->finger_count == 0) {
							if (jiffies > (ts->timestamp + 15 * HZ))
								confirm_calibration(ts);
						}
						if (ts->finger_data[report_type].y > 650
							&& (abs(ts->finger_data[report_type].x - ts->pre_data[report_type + 1]) > 400)) {
							if (report_type == 0 && ts->finger_count == 0)
								confirm_calibration(ts);
							i2c_atmel_write_byte_data(ts->client,
								get_object_address(ts, GEN_COMMANDPROCESSOR_T6) + 2, 0x55);
						}
				...
				if (((ts->grip_suppression >> report_type) & 1) == 0) {
					if (!ts->first_pressed)
						printk(KERN_WARNING "S%d@%d,%d\n", report_type + 1,
							ts->finger_data[report_type].x, ts->finger_data[report_type].y);
					ts->finger_count++;
					ts->finger_pressed |= 1 << report_type;
					if (ts->id->version >= 0x20 && ts->pre_data[0] == 0)
						ts->pre_data[report_type + 1] = ts->finger_data[report_type].x;
				}
			}
		} else {
			if (data[0] == get_report_ids_size(ts, GEN_COMMANDPROCESSOR_T6)) {
				printk(KERN_WARNING "Touch Status: ");
				msg_byte_num = 5;
			} else if (data[0] == get_report_ids_size(ts, PROCI_GRIPFACESUPPRESSION_T20)) {
				if (ts->calibration_confirm < 2 && ts->id->version == 0x16)
					check_calibration(ts);
				ts->face_suppression = data[1];
				printk(KERN_INFO "Touch Face suppression %s: ",
					ts->face_suppression ? "Active" : "Inactive");
				msg_byte_num = 2;
			} else if (data[0] == get_report_ids_size(ts, PROCG_NOISESUPPRESSION_T22)) {
				if (data[1] == 0x10) /* reduce message print */
					msg_byte_num = 0;
				else {
					printk(KERN_INFO "Touch Noise suppression: ");
					msg_byte_num = 4;
					if (ts->status && data[2] >= ts->GCAF_sample) {
						i2c_atmel_write_byte_data(ts->client,
					...
					if (data[1] & 0x0C && ts->GCAF_sample == 63) {
						ts->noisethr_config += 3;
						if (ts->noisethr_config > ts->noisethr && ts->noisethr)
...
static void cable_tp_status_handler_func(int connected)
{
	...
	if (connected != ts->status) {
		...
		if (ts->config_setting[1].config[0]) {
			if (ts->status && ts->id->version < 0x20) {
				ts->calibration_confirm = 2;
				i2c_atmel_write_byte_data(ts->client,
					get_object_address(ts, GEN_ACQUISITIONCONFIG_T8) + 6,
					ts->config_setting[ts->status].config_T8[6]);
			...
			if (!ts->status) {
				i2c_atmel_write_byte_data(ts->client,
					get_object_address(ts, PROCG_NOISESUPPRESSION_T22) + 8,
					ts->config_setting[0].config[1]);
				ts->noisethr_config = ts->config_setting[1].config[1];
			}
			i2c_atmel_write_byte_data(ts->client,
			...
			if (ts->config_setting[ts->status].config_T22 != NULL) {
				...
				ts->noisethr_config = ts->config_setting[ts->status].config_T22[8];
			}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		if (pdata->cable_config[0]) {
			...
			ts->GCAF_sample = ts->config_setting[1].config[3];
			if (ts->id->version >= 0x20)
				ts->noisethr = pdata->cable_config[0] - pdata->config_T9[31];
			else
				ts->noisethr = pdata->cable_config[0];
			ts->noisethr_config = ts->config_setting[1].config[1];
		} else {
			...
			if (pdata->cable_config_T9[0]) {
				...
				ts->GCAF_sample = ts->config_setting[1].config_T28[4];
			}
			if (ts->status)
				ts->noisethr = (ts->id->version >= 0x20) ?
					pdata->cable_config_T9[7] - pdata->cable_config_T9[31] :
					pdata->cable_config_T9[7];
			else
				ts->noisethr = (ts->id->version >= 0x20) ?
					pdata->config_T9[7] - pdata->config_T9[31] :
					pdata->config_T9[7];
			ts->noisethr_config = pdata->cable_config_T22[8];
